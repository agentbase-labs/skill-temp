#!/usr/bin/env node
/**
 * Delete files from the BACKEND Git repository (post-deploy).
 *
 * Calls POST /backend/delete-files (workflow-scoped — resolves the backend repo
 * via workflow metadata.backend_repo_id). Mirrors update-backend-code.cjs
 * conventions.
 *
 * Args:
 ...
 *
 * ⚠️ Deletion only updates Git — it does NOT redeploy. Run redeploy-backend.cjs
 *    after to publish the change to the live backend.
 *
...
if (!wf || paths.length === 0) {
  console.error('Usage: node delete-backend-files.cjs --workflow-id <id> \\');
  console.error('         --path <p> [--path <p> ...] [--message "msg"]');
  console.error('');
  console.error('Deletes files from the BACKEND repo. Does NOT redeploy —');
  console.error('run redeploy-backend.cjs afterward to publish.');
  process.exit(1);
}
...
(async () => {
  let r;
  try {
    r = await api('/backend/delete-files', 'POST', {
  ...
  if (r.success && commit.sha) {
    console.log(`✅ Deleted ${deleted.length} backend file(s) — commit ${commit.sha}`);
  ...
  console.log('');
  console.log('   Deletion does NOT redeploy — run redeploy-backend.cjs to publish the change.');
